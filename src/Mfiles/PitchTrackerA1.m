function [yr,r,d,cmd,dmin]=PitchTrackerA1(x,fs,dn)
  # Pitch estimator using Equation A1 of the reference.
  # Inputs;
  #   x - signal
  ...
  #   [yr,r,d,cmd,dmin]=PitchTrackerA1(sin200,40000,4);
...
if (nargin ~= 3) || (nargout ~= 5)
  print_usage ("[yr,r,d,cmd,dmin]=PitchTrackerA1(x,fs,dn)");
endif
...
for ds=1:rS
  r0(ds)=sum(yr(s+ds).^2);
  for T=1:(maxTS/2)
    r(ds,T)=sum(yr(s+ds-T).*yr(s+ds+T));
  endfor
endfor

# Step 2: Difference function
# Ignore difference for tau=0
d=zeros(rS-(maxTS/2), maxTS/2);
for ds=1:rS-(maxTS/2)
  for t=1:(maxTS)/2
...
cmd=zeros(size(d));
for ds=1:rS-(maxTS/2)
  cs=cumsum(d(ds, :))./(1:(maxTS/2));
  cmd(ds, :)=d(ds, :)./cs;
endfor

# Step 4: Absolute threshold
# Step 5: Parabolic interpolation
dmin=zeros(1,rS-(maxTS/2));
for ds=1:rS-(maxTS/2)
  for t=2:(maxTS/2)-1
