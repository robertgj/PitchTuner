...
playback_callback (snd_pcm_sframes_t nframes)
{
  ...
  /* ... fill buf with data ... */
  snd_pcm_uframes_t frames = (snd_pcm_uframes_t)nframes;
  snd_pcm_sframes_t sframes = 0;
  if ((sframes = snd_pcm_writei (playback_handle, buf, frames)) < 0) {
    err = (int)sframes;
...
main (int argc, char *argv[])
{
  ...
  if ((err = snd_pcm_hw_params_set_rate_near (playback_handle, hw_params, &frameRate, 0)) < 0) {
    ...
    */
    ...
    if ((frames_to_deliver = snd_pcm_avail_update (playback_handle)) < 0) {
      if (frames_to_deliver == -EPIPE) {
        ...
        fprintf (stderr, "unknown ALSA avail update return value (%u)\n", 
